...
var getItemMySQLJson = function(context, webView, cb) {
	...
	// Load foreign keys
	var query2 = "SELECT TABLE_NAME,TABLE_SCHEMA,COLUMN_NAME,CONSTRAINT_NAME,REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA='" + schema + "' AND REFERENCED_TABLE_NAME IS NOT NULL ORDER BY ORDINAL_POSITION;";
...
var getItemPostgreSQLJson = function(context, webView, cb) {
	var schema = context.currentSchema();
	...
	context.execute(query2, res => {
		res.rows.forEach(row => {
			var jsonItem = {"from": {"name": remoteQuoteIfNeeded(row.raw("child_name")),"schema": remoteQuoteIfNeeded(row.raw("child_schema")), "rows": []}, "to": {"name": remoteQuoteIfNeeded(row.raw("parent_name")), "schema": row.raw("parent_schema"), "rows": []}};
...
var getItemSQLServerJson = function(context, webView, cb) {
	var schema = context.currentSchema();
